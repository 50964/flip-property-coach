...
export async function POST(req: Request) {
  try {
    ...
    // Create privileged supabase client using service role key
    ...
    // Rate limit admin actions by actor id
    const { paymentLimiter: _pl, adminLimiter } = await import('@/lib/rate-limiter')
    ...
    // Perform update
    const { error } = await svc.from('profiles').update({ role: parsed.role }).eq('id', parsed.userId)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })

    // Audit log (best-effort)
    await svc.from('admin_audit').insert([{ action: 'promote_user', actor: user.id, target: parsed.userId, meta: { role: parsed.role }, created_at: new Date().toISOString() }])
